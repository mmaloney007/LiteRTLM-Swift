...
public final class LiteRTLMEngine: @unchecked Sendable {
    ...
    /// executor is initialized. Passing a non-nil value with a model that has
    /// no vision section will cause engine init to fail in the C++ runtime, so
    /// only set this when the loaded `.litertlm` actually contains a vision
    /// encoder (e.g. Gemma 4 vision variant).
    ...
    ///     `nil` for text-only models. Only pass non-nil when the `.litertlm`
    ///     file contains a vision encoder section; passing a value for a
    ///     text-only model causes engine init to fail.
            ...
            let createdEngine = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<OpaquePointer, any Error>) in
                self.inferenceQueue.async {
                    do {
                        ...
                        // where vision/audio are NULL for models that don't
                        // include those encoders. Passing the same `backendStr`
                        // for vision/audio on a text-only model causes the
                        // C++ runtime to attempt to load missing vision/audio
                        // sections from the .litertlm file and return NULL
                        // from settings_create. (Bug fix in 1.0.0-b4-asc.2.)
    ...
    fileprivate static func withOptionalCStrings<R>(
        ...
        // Materialize each string into its own scoped C buffer (or nil).
        // We nest withCString calls because Swift's pointer lifetime rules
        // require pointers to be used only inside the closure that produced
        // them.
        if let s1, let s2 {
